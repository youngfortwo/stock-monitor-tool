...
./bin/run_stock_tool_batches.sh "${TOTAL}" "${BATCH_SIZE}" > start_stock_tool_data_refresh.log 2>&1 &
REFRESH_PID="$!"
...
echo "Open: http://localhost:${PORT}/static/stock_dashboard.html"
echo "Refresh progress is shown on the dashboard and logged to start_stock_tool_data_refresh.log"
echo

PORT="${PORT}" "${PYTHON_BIN}" -W ignore::FutureWarning api/stock_server.py
