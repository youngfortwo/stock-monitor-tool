...
/usr/bin/python3 core/market_breadth.py

echo "Scanning trend candidates, limit=${LIMIT}..."
/usr/bin/python3 core/stock_scanner.py --limit "${LIMIT}" --output test_candidates.csv
...
if [[ -n "${TOP_CODE}" ]]; then
    /usr/bin/python3 core/plot_kline.py --symbol "${TOP_CODE}" --days 120 --output "kline_${TOP_CODE}.png"
else
    echo "No trend candidate found; skipped K-line chart."
fi

echo "Scanning SEPA Stage 2 candidates, limit=${LIMIT}..."
/usr/bin/python3 core/sepa_stage2_scanner.py --limit "${LIMIT}" --output sepa_stage2_candidates_test.csv
